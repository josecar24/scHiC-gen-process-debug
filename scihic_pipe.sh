...
mkdir -p $outdir #Make outdir
rm $outdir/*.html #And clear out all the results files
#Demultiplex reads by inline barcodes
SeqPrep -A AGATCGGAAGAGCGATCGG -B AGATCGGAAGAGCGTCGTG -f $tmp_dir$fq_r1  -r $tmp_dir$fq_r2 -1 $tmp_dir$fq_r1.clipped -2 $tmp_dir$fq_r2.clipped > seq_prep.txt 2>> adaptor_clipping_stats
python inline_splitter.py $tmp_dir$fq_r1.clipped $tmp_dir$fq_r2.clipped $barcodes $tmp_dir$fq_r1.split $tmp_dir$fq_r2.split 2> $outdir/splitting_stats.html
#Run analyze_scDHC_V2design.py, which searches for the adaptor and clips it out
python analyze_scDHC_V2design.py $bc1 $tmp_dir$fq_r1.split $tmp_dir$fq_r2.split $tmp_dir$fq_r1.bc_clipped $tmp_dir$fq_r2.bc_clipped > $outdir/$bc_assoc
#Align reads to combo-reference using bowtie2
bowtie2 -x combo_hg19_mm10 -p 4 -U $tmp_dir$fq_r1.bc_clipped 2> $outdir/$fq_r1.mapping_stats | samtools view -bS - > $tmp_dir$fq_r1.bam&
bowtie2 -x combo_hg19_mm10 -p 4 -U $tmp_dir$fq_r2.bc_clipped 2> $outdir/$fq_r2.mapping_stats | samtools view -bS - > $tmp_dir$fq_r2.bam&
wait
#Compute total # of bc_clipped reads and store that value
wc -l $tmp_dir$fq_r1.bc_clipped > $bc_assoc.ph1
read reads_r1 junk < $bc_assoc.ph1
total_reads=`expr $reads_r1 / 4`
#Housekeeping
rm $bc_assoc.ph1
#Convert sam files to bam
bedtools bamtobed -i $tmp_dir$fq_r1.bam > $tmp_dir$fq_r1.bed&
bedtools bamtobed -i $tmp_dir$fq_r2.bam > $tmp_dir$fq_r2.bed&
wait
cd $outdir
#Sort out all multimapping reads (MAPQ <= 30)
awk '$5 > 30' $tmp_dir$fq_r1.bed > $fq_r1.bed.mapq0&
awk '$5 > 30' $tmp_dir$fq_r2.bed > $fq_r2.bed.mapq0&
wait
rm $tmp_dir$fq_r1.bed &
rm $tmp_dir$fq_r2.bed &
rm $tmp_dir$fq_r1.clipped &
rm $tmp_dir$fq_r2.clipped &
rm $tmp_dir$fq_r1.bc_clipped &
rm $tmp_dir$fq_r2.bc_clipped &
...
python bin_schic.py combo_hg19_mm10.genomesize $bc_assoc.deduped.percentages.filterable $bedpe.mapq0.deduped.filtered > foo
rm foo
