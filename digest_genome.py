...
# Define restriction enzyme cut sites. '^' indicates the cutting point.
RE_cutsite = {
    "mboi": ["^GATC"],
    "dpnii": ["^GATC"],
    "bglii": ["A^GATCT"],
...
# Function to find restriction enzyme cut sites in a given FASTA file.
...
# Function to find chromosome lengths from the reference FASTA file
...
# Function to replace occurrences of 'N' with all possible nucleotides
def replaceN(cs):
    npos = int(cs.find('N'))
    cseql = []
    if npos != -1:
...
if __name__ == "__main__":
    ...
    for cs in cutsites:
        ...
        offpos = int(cseq.find('^')) # Determine cut position offset
        if offpos == -1:
    ...
    with open(out, 'w') as outfile:
        for chrom_name, indices in zip(contig_names, valid_fragments):
            ...
            for begin, end in indices:
                ...
                if end > begin:
                    frag_id += 1
                    frag_name = "HIC_{}_{}".format(str(chrom_name), int(frag_id)) #输出里面的第 4 列: 片段名称 (HIC_chr1_1, HIC_chr1_2, ...)
                    outfile.write("{}\t{}\t{}\t{}\t0\t+\n".format(str(chrom_name), int(begin), int(end), str(frag_name)))

"""
Command to run the script:

Assuming the reference genome file is "reference.fa" and you want to use the dpnii enzyme for cutting, and the output should be saved as "combo_hg38_ref.dpnii.bed":

python script_name.py reference.fa -r dpnii -o combo_hg38_ref.dpnii.bed
"""
